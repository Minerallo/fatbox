...
output_dir = './output/images/extracted'
os.makedirs(output_dir, exist_ok=True)
print(f"Directory {output_dir} is ready")  # Debugging statement
...
# sshfs -o "IdentityFile=/Users/ponsm/.ssh/id_rsa_hlrn" bbkponsm@glogin.hlrn.de:/scratch/usr/bbkponsm /Users/ponsm/Desktop/model_g -o defer_permissions
#vistimes = get_times('../../statistics')
vistimes = get_times('/Users/ponsm/Desktop/model_g/model_matias/15_PRM/15_wk_0.5/statistics')
print(vistimes)

XPIX=2000                 # X pixels from paraview export
Xlength=900                # Length of exported window in km %Length of the sampling bound
YPIX=200                  # Y pixels from paraview export
...
# User set variables.
endtime = float(0)*1e6
starttime = float(6)*1e6
x_pixels = int(XPIX)
y_pixels = int(YPIX)
xlength = float(Xlength)*1e3
strain_rate_factor = float(STRAIN_RATE_FACTOR)
minimum_distance = float(MINDISTANCE)
R = int(RAD)
num_proc = int(4)
min_fault_length = float(min_fault_length)*1e3
...
def extract_faults(file):
    ...
    # data = pandas.read_csv('/home/ponsm/Desktop/Fatboxinstall/Model_test/data_' + str(file) + '.csv', delimiter=',')
    data = pandas.read_csv('/Volumes/Tom/gmod20_save/Desktop/Fatboxinstall/Model_test/data_' + str(file) + '.csv', delimiter=',')

    print(data)

    # Get positions for fields we need.
   # nps_pos = data.columns.get_loc("noninitial_plastic_strain")
    nps_pos = data.columns.get_loc("plastic_strain")
    ...
    plot_components(G, ax=axs[1],node_size = node_scale)
    ...
    # plt.close("all")
    plt.savefig(fname=os.path.join(output_dir, 'image_' + str(file).zfill(5) + '.png'), dpi=200)
    plt.close("all")
